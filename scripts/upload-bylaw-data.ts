...
async function uploadBylawData() {
  ...
  // Log target so user can confirm DB and storage
  const dbDisplay =
    dbUrl.includes('supabase.com')
      ...
      : dbUrl.includes('localhost')
        ...
        : dbUrl.replace(/:\/\/[^@]+@/, '://***@')
  const storageDisplay = process.env.S3_BUCKET ? 'Supabase (S3)' : 'local'
  console.log(`Target: DB ${dbDisplay} | Storage: ${storageDisplay}\n`)
