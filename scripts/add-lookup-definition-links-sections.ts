// scripts/add-lookup-definition-links-sections.ts
// Walks bylawSections rich text blocks and wraps matching terms in link nodes
...
async function processSections(payload: Payload, regex: RegExp, aliasMap: Map<string, string>) {
  let page = 1;
  let updated = 0;
  while (true) {
    const res = await payload.find({
    ...
    for (const doc of res.docs) {
      ...
      if (changedBlocks) {
        try {
          ...
          updated += 1;
          console.log(`Updated bylawSection ${doc.id}`);
        } catch (err: any) {
          console.error('Failed to update bylawSection', doc.id);
          if (err?.data) {
  ...
  console.log(`Finished. Updated ${updated} sections.`);
}
...
async function start() {
  ...
  await processSections(payload, regex, aliasMap);
}
