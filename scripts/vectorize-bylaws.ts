/**
 * Vectorize bylaw subsections into bylaw_embeddings using OpenAI.
 ...
 *
 * Run: pnpm run vectorize:bylaws (or use vectorize-bylaws-airdrie.ts which also does sections)
 */
import './load-env'
import pg from 'pg'
...
async function run() {
  ...
  const slugs = subsections.map(s => s.slug)
  if (slugs.length > 0) {
    await pgClient.query(`DELETE FROM bylaw_embeddings WHERE slug = ANY($1::text[])`, [slugs])
    console.log('Cleared existing subsection embeddings for target bylaws')
  }

  for (const sub of subsections) {
    const text = blocksToText(sub.content)
    const combinedText = `${sub.code} ${sub.title || sub.label || ''}\n\n${text}`.trim()
